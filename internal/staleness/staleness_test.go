...
func TestScanReturnsNilWhenSkipped(t *testing.T) {
	...
	out, err := staleness.Scan(context.Background(), ".")
	if err != nil {
		t.Fatalf("Scan: %v", err)
	}
	if out != nil {
...
func TestScanActionsNoWorkflowsIsEmpty(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	out, err := staleness.Scan(context.Background(), dir)
	if err != nil {
		t.Fatalf("Scan: %v", err)
	}
	if len(out) != 0 {
...
func TestScanActionsParsesPinsFromYAML(t *testing.T) {
	...
	t.Setenv("LINTMAX_SKIP_STALENESS", "1")
	_, _ = staleness.Scan(context.Background(), dir) //nolint:errcheck // smoke test
}
