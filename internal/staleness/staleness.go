...
func Scan(ctx context.Context, root string) ([]Issue, error) {
	if os.Getenv(envSkip) == "1" { //nolint:forbidigo // reason: bootstrap layer owns env reads
		return nil, nil
	}
	var (
		actionsOut []Issue
...
func tolerance() time.Duration {
	if v := os.Getenv(envTolerance); v != "" { //nolint:forbidigo // reason: bootstrap layer owns env reads
		var n int
...
func doReleaseFetchUncached(ctx context.Context, owner, name, action string) (*ghRelease, error) {
	...
	if tok := os.Getenv(envToken); tok != "" { //nolint:forbidigo // reason: bootstrap layer owns env reads
